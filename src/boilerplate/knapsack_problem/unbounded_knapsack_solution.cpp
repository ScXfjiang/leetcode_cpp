...
// 1D DP Vector
int Knapsack1D_1(const std::vector<int>& weights, const std::vector<int>& values,
                 int capacity) {
...
// 1D DP Vector
int Knapsack1D_2(const std::vector<int>& weights, const std::vector<int>& values,
                 int capacity) {
  // dp[j] - maximum value with a capacity of j
  std::vector<int> dp(capacity + 1, 0);
  for (int j = 1; j <= capacity; ++j) {
...
int main() {
  std::vector<int> weights = {1, 2, 4, 5, 6};
  std::vector<int> values = {1, 3, 5, 6, 10};
  int capacity = 10;
  std::cout << Knapsack2D_1(weights, values, capacity) << std::endl;  // 16
  std::cout << Knapsack2D_2(weights, values, capacity) << std::endl;  // 16
  std::cout << Knapsack1D_1(weights, values, capacity) << std::endl;  // 16
  std::cout << Knapsack1D_2(weights, values, capacity) << std::endl;  // 16
  return 0;
}